...
*/
ob_start();
$admin = 'ripdio';//add_sudo
$idchannel = "thugmeme";//add_id_channel
define('API_KEY','891976667:AAF9OH7bcDyoKdPLYNPUh99NFJVORHqNhrQ');//add_token
$bot_support = "@ripdio";//add_bot_support
...
🆔 @php_seven","html","true");
	...
	$result = create_zip($files_to_zip,'@php_seven.zip');
	SendDocument($chat_id,new CURLFILE("@php_seven.zip"),'','📣 @php_seven
	🤖 @php_seven');
	unlink("@php_seven.zip");
	unlink("Group.txt");
	unlink("Member.txt");
	unlink("Start.txt");
	unlink("Send.txt");
...
<code>Welcome To My Bot:)</code>");
	...
	$down = json_decode(file_get_contents("https://api.feelthecode.xyz/instagram/?url=$text"));
		...
		$url = $down->url;
		if($down->is_video == true){
		SendVideo($chat_id,$url,"🆔 @php_seven",$button_official);
		}elseif($down->is_photo == true){	
		SendPhoto($chat_id,$url,$button_official,"🆔 @php_seven");
		}
		}else{
		save("other/$from_id/command.txt","downloader instagram");
